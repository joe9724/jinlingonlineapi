...
// NewServer creates a new api jinlinonline server but does not configure it
func NewServer(api *operations.JinlinonlineAPI) *Server {
	s := new(Server)
...
// Server for the jinlinonline API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/jinlinonline.sock"`
	...
	api          *operations.JinlinonlineAPI
...
func (s *Server) SetAPI(api *operations.JinlinonlineAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(2)
		s.Logf("Serving jinlinonline at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving jinlinonline at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(2)
		s.Logf("Serving jinlinonline at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving jinlinonline at http://%s", l.Addr())
		}(s.httpServerL)
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(2)
		s.Logf("Serving jinlinonline at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving jinlinonline at https://%s", l.Addr())
...
func (s *Server) Listen() error {
	...
	if s.hasScheme(schemeHTTP) {
		listener, err := net.Listen("tcp", net.JoinHostPort(s.Host, "97"))
		if err != nil {
			return err
		}
